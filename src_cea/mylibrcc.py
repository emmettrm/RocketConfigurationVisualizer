...
"""
NOTE: to do list
make ability to get array of cp along the nozzle
"""
...
channel_number_qt = 
IWT = 
constant_dim =   #'Largura da aleta', 'Largura do canal' NOTE: translation: 'fin width', 'channel width'
CCH = 
FT = 
Tc_primary = 
Twc_primary = 
...
def calc_prop(data_in, data_out, chems, cpArray):
    ...
    for i in reversed(range(0,data_out['size'])):
        ...
        #calculation fo static pressure
...
#iterations of the cooling channel calculations until it converges to a temperature
def iteration(data_in , data_out, chems, cpArray):
    ...
    for i in range(0,data_in['max_iterations']):
        ...
        calc_prop(data_in, data_out, chems, cpArray)
        if i==0: #first iteration edge case
            Tc_0 = sum(data_out['Q'])
...
def optimize_channel2(data_in, data_out, chems, cpArray):
    flag1 = False
    flag2 = False
    if data_in['dim_constant'] == 'FT':
        dim_const = 'FT'
    ...
    iteration(data_in, data_out, chems, cpArray)
    Q = max(data_out['Q'])
    Q_prev = Q
    Q0 = Q
    
    w = data_in['w']
    #iterates channel dimentions until a heat Q is converged upon
...
#---------------------------------------------------------------------------------------------------------------
"""
NOTE: output properties are: ro, cp, k, mi
also has a weird control with temperature for the kerosene but not ethanol

NOTE: list of data out values
output values:
...
data_out input values: data_out = {}

NOTE: data_in units are in the following from rccgui
